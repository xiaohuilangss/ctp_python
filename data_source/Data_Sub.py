...
#
#         current_price = float(
#             jq.get_price(stk_code_normal, count=1, end_date=get_current_datetime_str())['close'].values[0])
#
#     elif source == 'ts':
#         # 获取实时价格
...
def get_current_price_JQ(stk_code):
    ...
    print('当前price获取时间：%s' % datetime_str)
    # current_price = jq.get_ticks(security=stk_code_normal, count=1, end_dt=datetime_str)

    current_price = float(jq.get_price(stk_code_normal, count=1, end_date=datetime_str, frequency='1m')['close'].values[0])
...
if __name__ == '__main__':
    jq_login()
    while True:
        r = get_current_price_JQ('M2101.XDCE')
